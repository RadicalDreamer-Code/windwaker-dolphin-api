...
namespace Client.Dolphin {
    ...
    public class DolphinAccessor {
        ...
        public bool read(long offset, MemoryType type, out byte[] buffer, bool reverseBytes = false, uint length = 0) {
            if (this.Status == DolphinStatus.Hooked) {
                return this.Process.read(offset, type, out buffer, reverseBytes, length);
            }
