...
namespace Client.Dolphin {
    class WindowsDolphinProcess : DolphinProcess {
        ...
        public override bool read(long offset, MemoryType type, out byte[] buffer, bool reverseBytes = false, uint length = 0) {
            IntPtr ptr = new IntPtr((long)this.EmuStartAddress + offset);
            IntPtr bytesRead = IntPtr.Zero;
            int bufferLength = (int)length; // Common.getMemoryTypeSize(type, length);
